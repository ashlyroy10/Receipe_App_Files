...
const Chinese = () => {
  ...
  const fetchRecipe = () =>{
    axios.get("http://localhost:8501/recipe/readc")
    .then((response) => {
  ...
  if(updatestate){
    ...
    final = <div>
      <br></br><br></br><br></br>
      <Typography variant='h3'sx={{fontFamily:'cursive'}}>Chinese Cuisines </Typography><br></br><br/>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', alignItems: 'start' }}>
    {recipe.map((value, index) => (
      <Box key={index} sx={{ ml: 5, width: 300, height: 270, border: 1 }}>
        <br/>
